...
## 同源站使用场景（2026-09-17 补记，S2-25 / R5 —— **口径待定版**）

本软件侧栏外链的 **med-review-site** 是**公网服务**，与「本地桌面应用」是不同的使用场景：

| 场景 | 是否触发 AGPL §13 | 依据 |
|---|---|---|
| **本软件自身**（本地桌面版） | **不触发** | 仅绑回环（`run_medkit.py`）+ Host 守卫（`medkit/main.py`），不通过计算机网络供他人远程交互 |
| **med-review-site**（公网） | **取决于「同源」语义——待产品/法务定版** | 天然满足 §13 的「远程交互」要件；是否触发取决于站点是否**分发/运行了本软件的修改版** |

⚠️ 本文件**不预设结论**。两种读法与待办事项见 `docs/adr/ADR-007-license-agpl.md` 的
「补记（2026-09-17）」一节：若站点复用本仓 `medkit/web` / `render` 代码（读法 B），
则站点侧须显著提供 Corresponding Source 并附 AGPL §5 修改声明；
若仅按同一 JSON schema 与本软件产物互通（读法 A），则 §13 不触发。

**在定版前，请勿把本表当作站点合规结论使用。**
