...
> 生成日期：2026-09-15（版本基线 v0.10.2）。

## ⚠️ 需注意的许可证（分发前必读）

| 组件 | 许可证 | 影响 |
|---|---|---|
| **PyMuPDF** | **AGPL-3.0 或 Artifex 商业许可（双授权）** | AGPL 具**强传染性**：以 AGPL 组件提供的网络服务/分发产物，通常要求整体以 AGPL 开源。本项目目前**未声明许可**（见仓库根 `LICENSE`）；若以非开源方式分发安装包，需评估是否购买 Artifex 商业授权，或替换为宽松许可的 PDF 解析方案。**这是本清单中最需要产品决策的一项。** |
...
2. 新增依赖时按 `borrow-rules.md` §1 记录「名称 / 版本 / 许可证 / 体积」，并更新本文件与
   `requirements.lock`；
3. `PyMuPDF` 的 AGPL 决策（购买商业授权 / 接受 AGPL 开源 / 替换实现）**须在产品层面定案**
   并记入 ADR，不宜长期悬置。
