...
> IMP-13 待用户提供 306 大纲 PDF。本文件已随 S1 审查全套归档至 `docs/reviews/s1-2026-08-27/`。
...
- **步骤**：既有 MinerU 通道 → md → `chat_json`（挂 IMP-03 schema）抽取 10 条抽样人核对 ≥80% → 通过则 `/api/syllabus/parse` 增加文件输入路径；不通过则关闭此路线并更新 `docs/spikes/` 记录。
- **执行结果**：PDF → 文本转换（OCR 抽样 6 页一致性核验通过）→ `docs/spikes/k3_out/306大纲_2026.md` → 按「第四部分 考查内容」切 6 科逐科 `chat_json`（契约 `OutlineSubject`，见 `medkit/core/schema.py`；提示词 `medkit/prompts/syllabus_extract.md`）→ 独立解析器作真值全量核验：**条目 recall 100.0%（402/402）· precision 96.5% · 章名 66/75 · 10 条抽样一致 10/10（≥80% 通过）**。关键修复：推理型模型（deepseek-v4-flash）`max_tokens` 须 ≥16000（reasoning 占用输出预算，6000 时大科返回空）；科目名随正文下发避免误猜。
- **落地**：`/api/syllabus/seed/parse-file`（预览）与 `/api/syllabus/seed/import-file`（落库 source='seed'，幂等），LLM 不可用回退本地规则；spike 记录见 `docs/spikes/K3_syllabus_extract.py` + `docs/spikes/k3_out/`。
