...
#### ~~M-5（R6-12）提示词治理（NX-06）被违反【中】~~ → **复核撤注：NX-06 实际合规**

- **撤回原因（2026-09-15 12:0x 复核）**：初稿据「`medexplain.md`/`medtutor.md` 在 `8e69642`（08-29）改动」
  推断其随 0.10.0（08-31）发布，从而判定 0.10.0 缺少 `### Prompts` 小节。逐版本核验后**该推断错误**：
  `8e69642` **属于 v0.9.0**，其提示词改动归 0.9.0 小节（该版**有** `### Prompts`），
  而 v0.9.0 之后各版本**没有任何提示词改动**。
- **核验命令与结果（替代原证据）**
  ```bash
  git merge-base --is-ancestor 8e69642 v0.9.0 && echo "8e69642 已在 v0.9.0 内"   # → 是
  for r in "v0.8.0 v0.9.0" "v0.9.0 v0.10.0" "v0.10.0 v0.10.1" "v0.10.1 HEAD"; do
    set -- $r; printf "%s..%s 提示词提交数: " "$1" "$2"; git log --oneline "$1..$2" -- medkit/prompts/ | wc -l
  done
  # → 1 / 0 / 0 / 0
  grep -n "^## \|^### Prompts" CHANGELOG.md
  # → 0.8.0(518) 与 0.9.0(304) 均有 Prompts 小节，恰为两个有提示词改动的版本
  ```
- **结论**：**NX-06 合规**（每个改动提示词的版本都有 `### Prompts` 小节；`tests/fixtures/llm_cases/`
  7 个 fixtures 齐备）。**R6-12 撤销，不列入改进项**。
- **留档理由**：与 R5 报告「复核撤注（防重复误报）」同一做法——把误报的成因（把「提交日期」当作
  「版本归属」）写下来，避免后续轮次重复报出同一伪问题。**教训：版本归属必须用
  `git merge-base --is-ancestor <commit> <tag>` 判定，不能靠提交日期与发版日期接近来推断。**
...
> 并行分组：**A 组** R6-03 ∥ **B 组** R6-05 ∥ **C 组** R6-11 + R6-15（同为文档/闸门，无代码交集）
...
#### ~~R6-12 补记 NX-06 提示词小节【中 · 0.1 天】~~ → **撤销（见 §2.3 M-5 复核撤注）**

- **撤销原因**：逐版本核验证明 NX-06 **本就合规**——`8e69642` 属 v0.9.0（该版有 `### Prompts` 小节），
  v0.9.0 之后各版本无任何提示词改动。**无需任何 CHANGELOG 改动。**
- **保留此项的唯一价值**：作为「误报留档」，说明版本归属必须用
  `git merge-base --is-ancestor <commit> <tag>` 判定，不能靠提交日期推断。
...
- **涉及文件与修改点**
  1. 新增 `package.json`（devDependencies：`eslint`）+ `.eslintrc.json`
     （**勘误 2026-09-16：实际落地为 `eslint.config.js` flat config**）
...
| **3（工程品质）** | R6-03 ∥ R6-05 ∥ R6-11+R6-15 | **完全可并行**（无共同文件） | R6-04 排在 R6-07 之后；R6-12 已撤销 |
