...
- **S3-15 版本「五件套」守卫补全 + README 漂移修正**：原守卫只覆盖
  `__init__.__version__` / `APP_VERSION` / `pack/version.iss` **三件**——CHANGELOG 与 README
  无人看管，于是 0.10.4 已在代码/CHANGELOG/iss 声明，README 却仍写 v0.10.3（漂移正是这么漏出去的）。
  现补齐为**四件声明侧一致** + 两条用户可见性守卫：
  ① `CHANGELOG` 必须有 `## [<version>]` 段；② `README` 必须提及当前版本；
  ③ README 里让用户双击的安装包名**必须真实存在**于 `dist-installer/`（否则用户下载即 404）；
  ④ 若当前版本**尚无安装包**，README 必须显式标注「未发布 / 构建中」。
- **README 口径修正**：`已实现功能` 段版本号 v0.10.3 → **v0.10.4**；安装步骤补注
  「当前**已发布**的是 0.10.3，0.10.4（R8+W 修复版）**尚在构建中**，未发布」——
  避免用户以为下载到的就是最新修复版（2026-09-16「同号不同物」事故正是这类信息不对称）。

### 数据目录权限与备份恢复路径（Fixed，R8+W P2 收尾）

- **S3-13 数据目录权限加固**：新增 `config.harden_config_dir()`，在启动时对 `~/.medkit` 执行：
  - **POSIX**：目录权限过宽（组/其他可读或可写）→ `chmod 0700`（本应用自有目录，低风险高收益）；
  - **Windows**：**只检查、不修改**——读 `icacls`，若 `BUILTIN\Users` / `Everyone` 可访问则返回
    **可操作告警**（含建议命令）。不程序化改 ACL 的原因：Windows ACL 继承规则复杂，误删继承项
    可能把用户自己锁在目录外（低收益、高风险），故只提示不动手。
  - 踩坑留档：`icacls` 输出走**系统代码页**（中文 Windows 是 GBK），按 UTF-8 `text=True` 解码会在
    读取线程抛 `UnicodeDecodeError` → `stdout` 变 `None` → 检查**静默失效**；改为取字节后
    `decode("utf-8", errors="ignore")`。
- **S3-20 备份恢复的用户可见路径（安全收口）**：`restore` 自动端点**维持不提供**——模块 docstring
  早已记载理由（运行中覆盖 SQLite 有 WAL/连接缓存一致性问题，需**重启令牌机制 + 产品交互定案**），
  本批复核后**不推翻该决策**（半成品自动恢复的数据损坏风险远大于收益）。改为：
  ① `backup` 回执新增 `restore_hint`，把**手动恢复步骤**（退出应用 → 解压覆盖 → 重启）与
  不可逆风险提示明确写给用户（原只有「已备份到 X」，用户无从下手）；
  ② `downgrade_to` 的 docstring 说明它**有意不接入任何路由**（schema 回滚会丢列，无配套数据回滚
  等于静默丢数据；保留供人工排障），并写明将来接线的两个前置条件。

### 提示词一致性 / 出题硬规则 / 密钥提示 / 上传内容闸（Fixed，R8+W P2 收尾批）

- **S3-9 提示词占位符真正渲染**：`syllabus_extract.md` 声明了 `{subject_text}` 输入，原实现却用
  `load_prompt` **原样加载**（占位符永不替换），正文改由 user 消息下发——系统提示里长期挂着一个
  字面量 `{subject_text}`，与 medexplain/medgen/medtutor 的写法不一致。现改走 `render_prompt`
  真正渲染，user 消息退化为触发语。并加**全目录守卫**：任何含 `{var}` 占位符的提示词
  都不得被裸 `load_prompt` 加载（R3-16 教训：不要只盯一个文件）。
- **S3-8 `medgen.md` 补「不确定即不出题」硬规则**：原 HC-5 只说「不编造，改用题干陈述出处或
  **避开该细节**」——"避开"给了模型绕过去的空间。现明确：若正确答案无法由给定素材确定，
  **不出这道题**，宁可比配额少出，也不得换说法蒙混；少出的缺口由管线处理，**不接受凑数题**。
- **S3-10 DPAPI 回退明文改为常驻告警**：原仅在加密失败时弹一次性 toast，重启后用户以为已加密。
  新增 `config.security_warning()`（`plaintext` / `decrypt_failed`），供配置页/诊断常驻展示；
  非 Windows / 缺依赖路径同样置位（原该路径**完全不置位**）。
- **S3-12 解密失败不再回吐密文**：原 `_unprotect` 失败时**原样返回 `dpapi:xxx`**，该串会被当作
  API Key 发出去 → 用户看到莫名其妙的 401（故障提示错位）。现一律返回空串 + 留痕，
  调用方走既有的「未配置 Key」提示路径。
- **M2-04 上传补内容闸**：原只校验「扩展名 + 体积」，可上传**改名文件**（`.exe`→`.docx`）与
  **压缩炸弹**（几十 KB 的 docx 解压出几 GB），解析线程被长时间占用（本机 DoS）。现新增：
  ① 魔数闸（pdf/docx/png/jpg/bmp/webp，纯文本类型跳过）；② DOCX 的 ZIP 结构与解压体积/压缩比闸；
  ③ PDF **页数闸**（`MAX_PDF_PAGES = 2000`，在 `fitz.open` 处即拦）。

### Prompts

> **NX-06 义务说明**：本版改动 `medkit/prompts/medgen.md`（新增硬规则）与
> `medkit/prompts/syllabus_extract.md`（**加载方式**改为 `render_prompt`，文件本身未改）。
> 两者均**未改变输出契约的字段结构**，故 `tests/fixtures/llm_cases/`（medgen×3 等）无需同步。

- **`prompts/medgen.md`**（S3-8）：HC-5 由「不编造 + 可避开细节」收紧为「**答案无法由素材确定则不出该题**，
  宁可比配额少出，不接受凑数题」。影响：素材覆盖不足的切片会产出**少于配额**的题，
  由管线的题数缺口逻辑处理——这是有意的取舍（医学正确性优先于题量）。
- **`prompts/syllabus_extract.md`**（S3-9）：文件内容不变；**调用方**改为 `render_prompt`，
  使 `{subject_text}` 真正被替换（原先该占位符以字面量形式留在系统提示里）。

### 前端转义与合规标注（Fixed，R8+W P2 第二批）

- **S3-1 `learnChip()` 标签体补转义**：原 `class` 属性走了 `esc()`，但**标签体**没有——
  未知 state 时 `txt` 就等于入参本身 → 标签体注入面。现 `${esc(txt)}`。
- **S3-2 仪表盘「最近项目」`p.target` 补转义**：原 `+ p.target +` 直插（同行的 `p.pid` /
  `p.stage_label` 都已转义，只此一处漏）。
- **S3-3 Anki 导出文件名只取 basename**：原把服务端 `content-disposition` 里的文件名
  **原样**赋给 `a.download`（含路径分隔符/控制字符的名字会被按原样使用）。新增 `_baseName()`：
  剥掉路径与控制字符、空则回落到默认名。
- **S3-7 押题卷补「AI 生成、非官方真题」标注**：原产物页无任何来源声明，读者易当官方真题。
  现页头显著标注「本卷由 AI 依据教材/考纲生成，非官方真题，仅供自测参考」。
- **S3-11 删科目补清 `syllabus_items` / `realexam_freq`**：两表都带 `subject` 列，原删除清单漏了
  它们 → 删科目后残留孤儿行，**同名重建时旧条目「复活」**并污染覆盖度/考频权重。现一并删除并回显计数。
- **S3-14 更新检查加关闭开关**：原每次启动都会外呼 `api.github.com`，用户无从关闭。新增配置项
  `update_check`，置 `false` 时 `check()` 直接返回 `skipped` 且**不发起任何网络请求**（有专门用例守）。
- **M2-03 `regen` 补在飞去重**：原只有「管线运行中」检查 + per-pid 写锁 → 连点两次会**串行**拿到
  写锁各掷一次，同一道题被 LLM 重掷两遍（**双份计费**），前端按钮禁用挡不住并发端口。
  现按 `pid+qid` 走 `dedupe.begin/end`，第二次直接 409，且成功/失败路径都释放登记。
- **S3-16 CI 不再静默回退 `npm install`**：原 `npm ci … || npm install …` 会在 `npm ci` 失败时
  按 `package.json` 重新解析依赖树（**弱化 lock 约束、掩盖漂移**）。现 `npm ci` 失败即失败，
  并补 `npm audit --audit-level=high` 作告警信号（仅开发工具链，不进产物）。
