...
## 已实现功能（v0.4.0 · P1 迭代1~2 + 审查修复 + P2 路线图 + §5.4 网络检索 + 可玩性升级）
...
- **出题管线（五阶段，后台线程 + 实时日志）**：
  ...
  - ③ MedQC：LLM-as-judge 并行分批质检（D11/D16/D17/D18/D19/F1/F2），score + gate_decision
  - ④ MedFix：按 issue 定向修复（追溯日志）
  - ⑤ MedReview：分层复习手册（考点速记/易混淆/临床路径/数值速查/背诵清单）
  - ⑥ 渲染：题库 MD+HTML / **交互押题卷（X 型 checkbox+集合判分 / localStorage 续答 / 答题卡 / 计时 / 错题重练 / 打印）** / 复习手册 MD+HTML / **Anki 导出（.txt，HTML 安全转义）**
...
- 🔲 后续可选：多轮网络检索后端（bocha/Bing）、自备真题引用配额滑杆、B1 组题、.apkg 导出
